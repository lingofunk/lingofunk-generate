...
from lingofunk_generate.constants import (
    BATCH_SIZE_DEFAULT,
    DATA_FILE_PATH_DEFAULT,
    DROPOUT_DEFAULT,
    MAX_GEN_LENGTH_DEFAULT_CHAR_LEVEL,
    MAX_GEN_LENGTH_DEFAULT_WORD_LEVEL,
    MAX_LENGTH_DEFAULT_CHAR_LEVEL,
    MAX_LENGTH_DEFAULT_WORD_LEVEL,
    MODELS_FOLDER_PATH,
    NUM_EPOCHS_DEFAULT,
    TEXT_STYLES,
    TRAIN_SIZE_DEFAULT,
)
from lingofunk_generate.model_restore_utils import load_model, move_model_data
...
def _remove_folder_for_models_if_exists(models_folder):
    if os.path.exists(models_folder):
        log(
            'remove folder for models "{}" (as it may be not empty)'.format(
                models_folder
            )
        )
...
def _stratify_data(df, text_col, label_col, max_texts_per_label=None):
    ...
    for label in labels:
        ...
        texts_labels_current = list(
            zip(reviews_current[text_col].values, reviews_current[label_col].values)
        )

        texts_labels_current = texts_labels_current[
            :max_texts_per_label
        ]  # None also OK
...
def _fit_and_save_model(
    ...
    if max_length is None:
        max_length = (
            MAX_LENGTH_DEFAULT_WORD_LEVEL
            if word_level
            else MAX_LENGTH_DEFAULT_CHAR_LEVEL
        )
    if max_gen_length is None:
        max_gen_length = (
            MAX_GEN_LENGTH_DEFAULT_WORD_LEVEL
            if word_level
            else MAX_GEN_LENGTH_DEFAULT_CHAR_LEVEL
        )
...
def _parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        default=DATA_FILE_PATH_DEFAULT,
        help="Path to data .csv file",
    )
    parser.add_argument(
        "--text-col", type=str, required=True, help="Text column name in data file"
    )
    parser.add_argument(
        "--label-col",
        type=str,
    ...
    for text_style in TEXT_STYLES:
        parser.add_argument(
            "--labels-{}".format(text_style),
            action="append",
            type=int,
            required=False,
            help='Texts of which labels should be treated as ones of style "{}"'.format(
                text_style
            ),
        )
...
def _main():
    ...
    if not args.load_models:
        ...
        if not os.path.isdir(args.models_folder_source):
            raise IOError(
                'Can\'t load models from folder "{}": folder not exists'.format(
    ...
    data = _read_data_csv(
        args.data_path, cols=[args.text_col, args.label_col], nrows=args.nrows
    )
    ...
    for text_style in text_styles:
        ...
        ), 'No such text style "{}" in known styles "{}". Can\'t fit'.format(
